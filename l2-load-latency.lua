...
local PKT_SIZE	= 60
local ETH_DST	= "3c:fd:fe:05:b6:e6"
-- set addresses here
local DST_MAC       = "3c:fd:fe:05:b6:e6" -- resolved via ARP on GW_IP or DST_IP, can be overriden with a string here
local PKT_LEN       = 60
...
function master(args)
	...
	if dev1 ~= dev2 then
		mg.startTask("loadSlave", dev2:getTxQueue(0), DST_MAC)
	end
...
function loadSlave(queue, dstMac)
...
function timerSlave(txQueue, rxQueue, histfile)
	...
	while mg.running() do
		hist:update(timestamper:measureLatency(function(buf) buf:getUdpPacket().eth.dst:setString(ETH_DST) end))
	end
	hist:print()
	hist:save(histfile)
end
